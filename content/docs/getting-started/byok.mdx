...
We're a tiny, self-bootstrapping team building infrastructure for knowledge that matters. No venture capital, no pressure to scale before we're ready, no shortcuts on quality. Just a small group who believes deeply in user-owned knowledge and wants to build this right.

That means during beta, we're being careful. We can't afford to scale recklessly—both financially and architecturally. Instead, we're inviting people in batches, learning from each cohort, and making sure the infrastructure can handle growth before opening the floodgates.
...
Here's the other honest part: we can't pay for millions of AI processing requests. When you process documents in Recurse, we use *your* AI provider key to extract semantic structure, generate embeddings, and create the knowledge graph. You handle the compute costs, we provide the infrastructure.

This keeps Recurse free during beta while we figure out sustainable pricing. You only pay for what you actually process, and your knowledge graph lives in our infrastructure with full search, navigation, and retrieval capabilities at no additional cost. Most users spend less than $10 per month on AI processing, but it scales with your usage—process more documents, pay more to your AI provider.
...
<Accordions type="single">
  ...
  <Accordion title="What happens if I run out of AI credits?">
    Processing pauses until you add credits to your provider account. Your existing knowledge graph remains fully accessible for search and retrieval—you just can't process new documents until you add more credits.
  </Accordion>
