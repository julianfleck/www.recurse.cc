...
export function GraphView({
	...
	useEffect(() => {
		...
		return unsubscribe;
	}, [data, dataUrl, dataManagerRef.current]);
	...
	const _computeAndSetCurrentLevel = useCallback(() => {
	...
	const seedInitialPositions = useCallback(
		...
		[buildParentMap, computeSeedPosition, positionsRef.current.set],
	);
	...
	const cleanupAfterCollapse = useCallback(
		...
		[
			...
			healLayout,
			collapsingChildIdsRef.current.clear,
			collapsingNodesRef.current.delete,
			dataManagerRef.current,
			edgesToFadeRef.current.delete,
			isAnimatingCollapseRef,
		],
	);
	...
	const animateCollapse = useCallback(
		...
		[
			...
			allLinks,
			collapsingChildIdsRef,
			collapsingNodesRef.current.set,
			dataManagerRef.current,
			isAnimatingCollapseRef, // Mark this collapse as local to avoid double-trigger by external watcher
			localCollapseGuardRef.current.add, // Clear local guard once done
			localCollapseGuardRef.current.delete,
			positionsRef.current.get,
		],
	);
	...
	const createForceSimulation = useCallback(
		...
		[positionsRef, scheduleRender, simulationRef],
	);
	...
	}, [isInitialized, visibleNodes, setIsInitialized]);
	...
	}, [recenterToCurrentZoom, svgRef.current]);
	...
	}, [visibleNodes, layoutMode, focusedNodeId, svgRef.current]);
	...
	useEffect(() => {
		...
		fitAll,
		layoutTransitionRef,
		suppressLayoutFitUntilRef.current,
	]);
	...
	}, [isInitialized, visibleNodes, didInitialFitRef]);
	...
	const handleGlobalKeyDown = useCallback(
		...
		[fitAll, expandLevel, collapseLevel, toggleLayoutMode, isFullscreenOpen],
	);
