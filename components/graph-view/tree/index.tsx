...
export const TreeProvider = ({
  ...
  // Internal uncontrolled expansion state
  ...
  // Controlled expansion detection
  const isExpandedControlled =
    controlledExpandedIds !== undefined && onExpandedChange !== undefined;

  // Effective expanded set used by the tree
  const effectiveExpanded: Set<string> = isExpandedControlled
  ...
  const toggleExpanded = useCallback(
    ...
    [isExpandedControlled, effectiveExpanded, onExpandedChange]
  );
...
export const TreeIcon = ({
  ...
  const getDefaultIcon = () =>
    hasChildren
      ? isExpanded
        ? (iconOpen ?? <IconFolderFilled className="h-4 w-4" />)
        : (iconClosed ?? <IconFolder className="h-4 w-4" />)
      : (iconClosed ?? <IconFile className="h-4 w-4" />);
