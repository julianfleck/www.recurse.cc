  ...
  useEffect(() => {
    ...
    const updateParticles = () => {
      particlesRef.current.forEach((particle, index) => {
        ...
        const maxX = isViewportSized ? window.innerWidth : canvas.offsetWidth;
        const maxY = isViewportSized ? window.innerHeight : canvas.offsetHeight;

        if (particle.baseX < 0 || particle.baseX > maxX) {
          particle.vx *= -1;
        }
        if (particle.baseY < 0 || particle.baseY > maxY) {
          particle.vy *= -1;
        }

        // Keep base particles within bounds
        particle.baseX = Math.max(0, Math.min(maxX, particle.baseX));
        particle.baseY = Math.max(0, Math.min(maxY, particle.baseY));
