...
const buttonVariants = cva(
  "inline-flex shrink-0 items-center justify-center gap-2 whitespace-nowrap rounded-md border border-border font-medium text-sm outline-none transition-all focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/50 disabled:pointer-events-none disabled:opacity-50 aria-invalid:border-destructive aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 [&_svg:not([class*='size-'])]:size-4 [&_svg]:pointer-events-none [&_svg]:shrink-0",
  {
    variants: {
      variant: {
        default:
        ...
        subtle:
          "border-primary/20 bg-primary/10 text-primary hover:border-primary/30 hover:bg-primary/20",
...
function Button({
  ...
  // Default tooltipSide to "bottom" for icon buttons (size="icon" or "icon-sm")
  const defaultTooltipSide = tooltipSide ?? (size === "icon" || size === "icon-sm" ? "bottom" : "top");
  ...
  return (
    <Tooltip>
      ...
      <TooltipContent side={defaultTooltipSide} sideOffset={tooltipSideOffset}>
        {tooltip}
      </TooltipContent>
    </Tooltip>
  );
